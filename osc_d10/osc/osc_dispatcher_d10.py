...
"""
"""
d10 edit to clear the mapped adress from the list if it has no custom callbacks, 
so that the default callback would handle it.
"""
...
class D10Dispatcher(object):
    ...
    def unmap(self, address, handler, *args, needs_reply_address=False):
        try:
            ...
            if len(self._map[address]) == 0:
                # d10edit remove the adress if it has no custom callbacks so that the default one can handle it
                self._map.pop(address)
